...
static void _handle_midi_event(MIDI_MidiEvent event, Synth *synth) {
	...
	switch (event.status) {
		case MIDI_STATUS_NOTE_OFF: {
			...
			#endif
			if (voice_index != -1) {
				Synth_ReleaseChannelEnvelope(synth, voice_index);
		...
		case MIDI_STATUS_NOTE_ON: {
			if (voice_index != -1) {
				if (vel > 0) {
					...
					#endif
					Synth_SetChannelFrequency(synth, voice_index, _midi_to_freq(note));
					Synth_SetChannelVolume(synth, voice_index, ((float) vel) / 128);
					Synth_TriggerChannelEnvelope(synth, voice_index);
					voice_assign[voice_index] = note;
				} else {
					#ifdef MIDI_PRINT_INFO_MSGS
					printf("[MIDI](V-%02i) Note Off Event: Note %i, Velocity %i\n", voice_index, note, vel);
					#endif
					Synth_ReleaseChannelEnvelope(synth, event.channel);
					voice_assign[voice_index] = -1;
				}
			}
		} break;
	...
	for (event_index=0; event_index<CURR_TRACK.count; event_index++) {
		...
		if (CURR_EVENT.type == MIDI_EVENT_TYPE_META) {
			...
			switch (CURR_EVENT.meta.type) {
				...
				case MIDI_META_PORT: break; // Unimplemented
	...
	if (midi->header.format == MIDI_FMT_SIMULTANEOUS) {
		for (int i=0; i<midi->header.num_tracks; i++) {
			struct {
				...
				Synth *synth;
				Uint16 track_nr;
			} *track_data = malloc(sizeof(MIDI_File *) + sizeof(Synth *) + sizeof(Uint16));
...
MIDI_File *MIDI_OpenFile(char *filename) {
	MIDI_File *midi = malloc(sizeof(MIDI_File));
	FILE *file = Util_FS_Open(filename, "r");
	int index = 0;
	...
	while (state != DONE && !feof(file)) {
		...
		switch (state) {
			...
			case DELTA_TIME: {
				...
				if (event_index == CURR_TRACK.capacity-1) {
					CURR_TRACK.capacity += sizeof(MIDI_Event) * MIDI_EVENT_BUFFER_SIZE;
				...
				if (CURR_EVENT.type == MIDI_EVENT_TYPE_META) {
					...
					if (index > 1) {
						...
						if (i == CURR_EVENT.meta.length - 1) {
							...
							// Parse tempo if that's what this is
							if (CURR_EVENT.meta.type == MIDI_META_SET_TEMPO) {
								midi->uspq = 0;
								midi->uspq |= CURR_EVENT.meta.data[0]; midi->uspq <<= 8;
								midi->uspq |= CURR_EVENT.meta.data[1]; midi->uspq <<= 8;
								midi->uspq |= CURR_EVENT.meta.data[2];
...
void MIDI_PlayFile(MIDI_File *midi, Synth *synth) {
...
void MIDI_PrintInfo(MIDI_File *midi) {
